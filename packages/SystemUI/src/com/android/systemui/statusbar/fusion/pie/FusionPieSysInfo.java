 ...
 */
public class FusionPieSysInfo extends FusionPieSliceContainer implements ValueAnimator.AnimatorUpdateListener {

    private FusionPieController mController;
    ...
    public FusionPieSysInfo(Context context, FusionPieLayout parent,
            FusionPieController controller, int initialFlags) {
    ...
    private void setColor() {
        int textColor = (Settings.System.getInt(mContext.getContentResolver(),
                Settings.System.FUSION_PIE_TEXT_COLOR, -2));
        if (textColor == -2) {
            textColor = mContext.getResources().getColor(R.color.fusion_pie_text_color);
        }
    ...
    public void prepare(Position position, float scale, boolean mirrorRightFusionPie) {
        ...
        int textsize = res.getDimensionPixelSize(R.dimen.fusion_pie_textsize);
    ...
    public void draw(Canvas canvas, Position position) {
        ...
        if (mStaleData || Settings.System.getInt(mContext.getContentResolver(),
                Settings.System.FUSION_PIE_SHOW_TEXT, 1) == 0) {
            return;
        }
