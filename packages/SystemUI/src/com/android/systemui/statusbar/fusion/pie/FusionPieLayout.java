...
/**
 * This is the fusion pie layout container.
 * <p>
 * This class is responsible for displaying the content of a fusion pie control. And
 * processing the input events from the user.<br>
 * (It handles the events for the snap points, too.)
 */
public class FusionPieLayout extends FrameLayout implements View.OnTouchListener {
    public static final String TAG = "FusionPieLayout";
    ...
    // default value, must fit with FusionPieStyleSettings.java in settings
    public static final float FUSION_PIE_CONTROL_SIZE_DEFAULT = 0.84f;
    // default icon scale value on default control size
    // need to be >= FUSION_PIE_CONTROL_SIZE_DEFAULT
    public static final float FUSION_PIE_ICON_START_SIZE_FACTOR = 1.0f;
    ...
    private float mFusionPieScale;
    private boolean mMirrorRightFusionPie = true;
    ...
    private ValueAnimator.AnimatorUpdateListener mUpdateListener = new ValueAnimator.AnimatorUpdateListener() {
        ...
        public void onAnimationUpdate(ValueAnimator animation) {
            ...
            FusionPieLayout.this.invalidate();
        }

    };

    /**
     * A {@code FusionPieDrawable} is everything that can get displayed on the fusion pie control.
     * <p>
     * This defines the basic geometry of a fusion pie thing and provides the
     ...
     */
    public abstract static class FusionPieDrawable {
        protected float mStart;
        protected float mSweep;
        protected int mInner;
        protected int mOuter;

        abstract public void prepare(Position position, float scale, boolean mirrorRightFusionPie);

        abstract public void draw(Canvas canvas, Position position);

        abstract public FusionPieItem interact(float alpha, int radius);
        ...
        public final static int DISPLAY_NOT_AT_TOP = Position.LEFT.FLAG
                | Position.BOTTOM.FLAG
                | Position.RIGHT.FLAG;
        // The FusionPieDrawable is visible, note that slice visibility overrides item visibility
     ...
     */
    public abstract static class FusionPieSlice extends FusionPieDrawable {
        /**
        ...
        /**
         * The slice will be considerer as important - {@link FusionPieLayout} will try to keep
         * these slices on screen, when placing the fusion pie control.
         * @see FusionPieDrawable#flags
         */
    ...
    private List<FusionPieSlice> mSlices = new ArrayList<FusionPieSlice>();
    private List<FusionPieDrawable> mDrawableCache = new ArrayList<FusionPieDrawable>();
    private FusionPieItem mActiveItem;
    ...
    public FusionPieLayout(Context context) {
        super(context);
        ...
        mShowBackground = Settings.System.getInt(mContext.getContentResolver(),
                Settings.System.FUSION_PIE_SHOW_BACKGROUND, 1) == 1;
    }
    ...
    private void getDimensions() {
        mFusionPieScale = Settings.System.getFloat(mContext.getContentResolver(),
                Settings.System.FUSION_PIE_SIZE, FUSION_PIE_CONTROL_SIZE_DEFAULT);
        mMirrorRightFusionPie = Settings.System.getInt(mContext.getContentResolver(),
                Settings.System.FUSION_PIE_MIRROR_RIGHT, 1) == 1;

        final Resources res = mContext.getResources();

        mSnapRadius = res.getDimensionPixelSize(R.dimen.fusion_pie_snap_radius) * mFusionPieScale;
        ...
        mPadding = res.getDimensionPixelSize(R.dimen.fusion_pie_padding);
    }
    ...
    private void getColors() {
        ...
        int snapPaintColor = (Settings.System.getInt(mContext.getContentResolver(),
                Settings.System.FUSION_PIE_SNAP_COLOR, -2));
        if (snapPaintColor == -2) {
            snapPaintColor = res.getColor(R.color.fusion_pie_snap_color);
        }
        mSnapPaint.setColor(snapPaintColor);
        mSnapPaint.setStyle(Style.STROKE);
        mSnapPaint.setStrokeWidth(res.getDimensionPixelSize(R.dimen.fusion_pie_snap_outline));
        ...
        float backgroundAlpha = (Settings.System.getFloat(mContext.getContentResolver(),
                Settings.System.FUSION_PIE_BACKGROUND_ALPHA, 0.3f));

        int backgroundPaintColor = (Settings.System.getInt(mContext.getContentResolver(),
                Settings.System.FUSION_PIE_BACKGROUND_COLOR, -2));
        if (backgroundPaintColor == -2) {
            backgroundPaintColor = res.getColor(R.color.fusion_pie_overlay_color);
        }
    ...
    private void setupSnapPoints(int width, int height) {
        if (Settings.System.getInt(mContext.getContentResolver(),
                Settings.System.FUSION_PIE_SHOW_SNAP, 1) == 0) {
            mActiveSnap = null;
    ...
    public void onDraw(Canvas canvas) {
        if (mActive) {
            ...
            // At the fusion pie's internal view the center is always at (0,0) compensate for that!
            ...
            for (FusionPieDrawable drawable : mDrawableCache) {
    ...
    public boolean onTouch(View v, MotionEvent event) {
        ...
        if (mActive) {
            if (action == MotionEvent.ACTION_DOWN) {
                ...
                // since everything is drawn with mFusionPieScale we need to take this into account,
                // since the FusionPieDrawables expect normalized coordinates.
                int radius = (int) (distance / mFusionPieScale);

                if (DEBUG_INPUT) {
                    Slog.d(TAG, "interact on: (" + alpha + "," + distance + ")");
                }

                FusionPieItem newItem = null;
                for (FusionPieDrawable drawable : mDrawableCache) {
                    FusionPieItem tmp = drawable.interact(alpha, radius);
                    if (tmp != null) {
                        newItem = tmp;
                    }
                }
    ...
    private void updateActiveItem(FusionPieItem newActiveItem, boolean newLongPressed) {
        if (mActiveItem != newActiveItem || mLongPressed != newLongPressed) {
            ...
            if (newActiveItem != null) {
                boolean canLongPressed = (newActiveItem.flags & FusionPieItem.CAN_LONG_PRESS) != 0;
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        int viewMask = FusionPieDrawable.VISIBLE | mPosition.FLAG;
        ...
        if (changed || mPosition != mLayoutDoneForPosition) {
            ...
            for (FusionPieSlice slice : mSlices) {
                slice.prepare(mPosition, mFusionPieScale, mMirrorRightFusionPie);
                if ((slice.flags & viewMask) == viewMask) {
                    ...
                    if (slice instanceof FusionPieSliceContainer) {
                        for (FusionPieItem item : ((FusionPieSliceContainer)slice).getItems()) {
                            if ((item.flags & viewMask) == viewMask) {
                                item.prepare(mPosition, mFusionPieScale, mMirrorRightFusionPie);
        ...
        for (FusionPieSlice slice : mSlices) {
            if ((slice.flags & viewMask) == viewMask && (slice.flags & FusionPieSlice.IMPORTANT) != 0) {
                estimatedWidth = Math.max(estimatedWidth, slice.estimateWidth());
            }
        }
        estimatedWidth = estimatedWidth  * mFusionPieScale;
    ...
    public void activate(Point center, Position position) {
        ...
        // Set the activation center as center of the fusion pie
    ...
    public void addSlice(FusionPieSlice slice) {
        mSlices.add(slice);
    }
