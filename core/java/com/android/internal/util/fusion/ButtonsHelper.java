...
public class ButtonsHelper {
    ...
    // get and set the fusion pie configs from provider and return propper arraylist objects
    // @ButtonConfig
    public static ArrayList<ButtonConfig> getFusionPieConfig(Context context) {
        return (ConfigSplitHelper.getButtonsConfigValues(context,
            getFusionPieProvider(context), null, null, false));
    }

    public static ArrayList<ButtonConfig> getFusionPieConfigWithDescription(Context context, String values, String entries) {
        return (ConfigSplitHelper.getButtonsConfigValues(context,
            getFusionPieProvider(context), values, entries, false));
    }

    private static String getFusionPieProvider(Context context) {
        String config = Settings.System.getStringForUser(
                    context.getContentResolver(),
                    Settings.System.FUSION_PIE_BUTTONS_CONFIG,
                    UserHandle.USER_CURRENT);
        if (config == null) {
            config = ButtonsConstants.NAVIGATION_CONFIG_DEFAULT;
        }
    ...
    public static void setFusionPieConfig(Context context, ArrayList<ButtonConfig> buttonsConfig, boolean reset) {
        String config;
        if (reset) {
            config = ButtonsConstants.NAVIGATION_CONFIG_DEFAULT;
        } else {
        ...
        Settings.System.putString(context.getContentResolver(),
                    Settings.System.FUSION_PIE_BUTTONS_CONFIG, config);
    }

    public static ArrayList<ButtonConfig> getFusionPieSecondLayerConfig(Context context) {
        return (ConfigSplitHelper.getButtonsConfigValues(context,
            getFusionPieSecondLayerProvider(context), null, null, false));
    }

    public static ArrayList<ButtonConfig> getFusionPieSecondLayerConfigWithDescription(Context context, String values, String entries) {
        return (ConfigSplitHelper.getButtonsConfigValues(context,
            getFusionPieSecondLayerProvider(context), values, entries, false));
    }

    private static String getFusionPieSecondLayerProvider(Context context) {
        String config = Settings.System.getStringForUser(
                    context.getContentResolver(),
                    Settings.System.FUSION_PIE_BUTTONS_CONFIG_SECOND_LAYER,
                    UserHandle.USER_CURRENT);
        if (config == null) {
            config = ButtonsConstants.FUSION_PIE_SECOND_LAYER_CONFIG_DEFAULT;
        }
        return config;
    }

    public static void setFusionPieSecondLayerConfig(Context context, ArrayList<ButtonConfig> buttonsConfig, boolean reset) {
        String config;
        if (reset) {
            config = ButtonsConstants.FUSION_PIE_SECOND_LAYER_CONFIG_DEFAULT;
        } else {
        ...
        Settings.System.putString(context.getContentResolver(),
                    Settings.System.FUSION_PIE_BUTTONS_CONFIG_SECOND_LAYER, config);
    }
